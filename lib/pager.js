...
function initialize(page) {
  Pager.cwp = 0;
  Pager.len = 0;
  Pager.page = []; // should be [ {String} ]?
  Pager.stack = [];

  if (isArray(page)) {
    Pager.page = page;
    Pager.len = page.length;
  }
  return Pager;
}
/**
...
Pager.__proto__ = EventEmitter.prototype;
...
Pager.move = function(index, relative) {
  // this._data?
...
Pager.add = function(target, item) {
  if (!isNumber(target)) return this;
  var index = this.cwp
    , len = this.len;
  target = Math.floor(target);
